...
エンチャントの`apply_impulse`を利用してプレイヤー・Mobのモーションを操作するデータパック
...
解消されていない。MC-303493とは別種の問題。本フォークがしている`6.gamemode_swap_enter`/
`_exit`（発火の瞬間だけ衝突判定のないゲームモードを経由する対策）は、この
Y軸消失バグに対する引き続き有効な回避策として維持している

**既知のトレードオフ**：プレイヤーのゲームモード切り替えはクライアント主導の
移動処理と競合しうるため、往復のタイミングによってはサーバーが直後に設定した
Motionがクライアント側の状態で上書きされ、ごく低い確率でMotion（特にY軸方向）
が消えてしまうことがある。これはネットワーク的な
タイミング次第でランダムに起こるもの。ひろばお氏PlayerMotionはこの切り替えを一切
行わずに動作しているため、原理的には無くても成立する処理だが、「ブロックの
隙間が無くても押し出したい／Y軸のMotionが消える問題を回避したい」ため実装している。

Mobはサーバーが位置を完全に管理しているため、
この問題は起こらないので一連の処理を省略している。
